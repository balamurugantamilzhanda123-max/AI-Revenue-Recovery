        ...
        results.append(
            {
                ...
                "payment_status": tx.status.value if hasattr(tx.status, "value") else str(tx.status),
                "recovery_status": tx.recovery_status.value if hasattr(tx.recovery_status, "value") else str(tx.recovery_status),
                "recovered_amount": float(tx.recovered_amount or 0.0),
                "escalation_status": tx.escalation_status.value if hasattr(tx.escalation_status, "value") else str(tx.escalation_status),
